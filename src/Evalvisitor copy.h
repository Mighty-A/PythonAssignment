...
class EvalVisitor : public Python3BaseVisitor {
    ...
    antlrcpp::Any visitFile_input(Python3Parser::File_inputContext* ctx) override
    {
        ...
        Variables.push_back(GlobalVaraible); //setting the global 
    ...
    antlrcpp::Any visitFuncdef(Python3Parser::FuncdefContext* ctx) override
    {
        ...
        if (Function.count(tmpFuncName)) { //calling the function
            ...
            /*
            if (tmp.is<int>()) {
                std::cout << tmpFuncName << ' ' << tmp.as<int>() << std::endl;
            }
            */
    ...
    antlrcpp::Any visitTypedargslist(Python3Parser::TypedargslistContext* ctx) override
    {
        ...
        int countOfNonDefaultArg = ctx->tfpdef().size() - ctx->test().size();
        while (i < ctx->tfpdef().size()) { //deal with the args that are assign or default
            ...
            if (Variables[Variables.size() - 1].count(tmpArgName) == 0) { //default;
                for (int j = 0; j < ctx->test().size(); j++) {
                    if (visit(ctx->tfpdef(j + countOfNonDefaultArg)).as<std::string>() == tmpArgName) {
    ...
    { //visit simple_stmt and compound_stmt
    ...
    */
    ...
    antlrcpp::Any visitSuite(Python3Parser::SuiteContext* ctx) override
    {
        if (ctx->simple_stmt()) {
            ...
            for (int i = 0; i < ctx->stmt().size(); i++) {
                ...
                if (tmp.is<int>()) {
                    int flow = tmp.as<int>();
                    if (flow == 1 || flow == 2 || flow == -1 || flow == 4) //break, continue, non-value return
                        return flow;
    ...
    antlrcpp::Any visitComparison(Python3Parser::ComparisonContext* ctx) override
    {
        if (ctx->comp_op().size() > 0) {
            ...
            */
            ...
            bool result = true;
            antlrcpp::Any left;
            antlrcpp::Any right = visit(ctx->arith_expr(arithIndex));
            for (int i = 0; i < ctx->comp_op().size(); i++) { //deal with comparison;
                int tmpComp = visit(ctx->comp_op(i)).as<int>();
                arithIndex++;
                left = right;
                right = visit(ctx->arith_expr(arithIndex));
                if (left.is<std::string>()) {
                    std::string tmp1 = left.as<std::string>();
                    std::string tmp2 = right.as<std::string>();
                    switch (tmpComp) {
                    case 0:
                        if (tmp1 >= tmp2)
                            result = false;
                    ...
                    BigInteger tmp1 = left.as<BigInteger>();
                    BigInteger tmp2 = right.as<BigInteger>();
                    switch (tmpComp) {
                    case 0:
                        if (tmp1 >= tmp2)
                            result = false;
                    ...
                    double tmp1 = left.as<double>();
                    double tmp2 = right.as<double>();
                    switch (tmpComp) {
                    case 0:
                        if (tmp1 >= tmp2)
                            result = false;
                ...
                } else if (!left.is<BigInteger>() || !right.is<BigInteger>()) { //one of the two is not BigInteger
                    double tmp1 = 0, tmp2 = 0;
                    if (left.is<BigInteger>())
                        tmp1 = double(left.as<BigInteger>());
                    else if (left.is<double>())
                        tmp1 = double(left.as<double>());
                    else if (left.is<bool>())
                        if (left.as<bool>())
                            tmp1 = 1;
                        else
                            tmp1 = 0;
                    if (right.is<BigInteger>())
                        tmp2 = double(right.as<BigInteger>());
                    else if (right.is<double>())
                        tmp2 = double(right.as<double>());
                    else if (right.is<bool>())
                        if (right.as<bool>())
                            tmp2 = 1;
                        else
    ...
    antlrcpp::Any visitArith_expr(Python3Parser::Arith_exprContext* ctx) override
    {
        if (ctx->ADD().size() == 0 && ctx->MINUS().size() == 0) { //no arith
            ...
            if (firstTerm.is<std::string>()) { //deal with string
                ...
                if (firstTerm.is<bool>() && firstTerm.as<bool>())
    ...
    antlrcpp::Any visitTerm(Python3Parser::TermContext* ctx) override
    {
        if (ctx->factor().size() > 1) {
            ...
            if (tmpfirstFactor.is<std::string>()) {
                ...
                antlrcpp::Any tmp = visit(ctx->factor(1));
                BigInteger tmpFactor;
                BigInteger one("1");
                if (tmp.is<BigInteger>())
                    tmpFactor = tmp.as<BigInteger>();
                else if (tmp.is<bool>() && tmp.as<bool>())
                    tmpFactor = one;
                else
                    tmpFactor = BigInteger("0");
    ...
    antlrcpp::Any visitAtom_expr(Python3Parser::Atom_exprContext* ctx) override
    {
        ...
        if (ctx->trailer()) { //function call
            ...
            if (funcName == "print") {
                ...
                } else if (returnValue.is<int>() && returnValue.as<int>() == 4) { //deal with the non-return func
                    return 0;
                }
                return returnValue;
            }
            ...
            if (value.is<std::string>() && (value.as<std::string>()[0] != '"' || value.as<std::string>()[0] != '\'')) { //the value is a variable name
                ...
                if ( Variables[Variables.size() - 1].count(variableName)) {
                    if (useVariableAsLvalue) {
                        antlrcpp::Any* value1 = &Variables[Variables.size() - 1][variableName];
                        return value1;
                    } else {
                        antlrcpp::Any value1 = Variables[Variables.size() - 1][variableName];
                        return value1;
                    }
                } else if (!useVariableAsLvalue && Variables[0].count(variableName)) {
                    antlrcpp::Any value1 = Variables[0][variableName];
                    return value1;
                } 
                /*else if (assigningArglist && Variables.size() > 1 && Variables[Variables.size() - 2].count(variableName)) {
                    antlrcpp::Any value1 = Variables[Variables.size() - 2][variableName];
                    return value1;
                } else if (assigningArglist && !useVariableAsLvalue && Variables[0].count(variableName)) {
                    antlrcpp::Any value1 = Variables[0][variableName];
                    return value1;
                }*/
            }
            //std::cout << value.is<BigInteger>() << std::endl;
            return value;
        }
    }
    ...
    {
        antlrcpp::Any tmp = visit(ctx->test());
        //std::cout << tmp.is<BigInteger>() << std::endl;
